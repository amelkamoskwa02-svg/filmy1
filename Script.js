// Baza danych powiązana z Twoimi linkami Google Drive
const contentData = {
  filmy: [],
  csi: [
    {
    ...
    {
      id: 18,
      title: "CSI: Odcinek 18 – NÜRBURGRING",
    ...
    {
      id: 21,
      title: "CSI: Odcinek 21 VIPER ACR",
    ...
    {
      id: 22,
      title: "CSI: Odcinek 22 Porsche MANTA",
...
function renderMovies() {
  ...
  let currentList = contentData[currentCategory] || [];
  
  // Wyszukiwarka live
  if (searchQuery.trim() !== '') {
    // Przeszukaj wszystkie kategorie w przypadku wpisania tekstu
    currentList = [];
    Object.keys(contentData).forEach(cat => {
      contentData[cat].forEach(item => {
        if (item.title.toLowerCase().includes(searchQuery.toLowerCase())) {
          currentList.push({ ...item, category: cat });
        }
      });
    });
  }
  ...
  if (currentList.length === 0) {
    moviesGrid.innerHTML = '<p style="color: #888; text-align: center; grid-column: 1/-1; padding: 30px;">Brak wyników do wyświetlenia.</p>';
    return;
  }
  ...
  currentList.forEach(item => {
    ...
    if (lastWatchedId && item.id.toString() === lastWatchedId.toString() && !searchQuery) {
      card.classList.add('watched');
    }

    const catKey = item.category || currentCategory;
    const savedTime = localStorage.getItem(`time_${catKey}_${item.id}`);
    ...
    card.addEventListener('click', () => {
      if (item.category) {
        ...
        document.querySelectorAll('.category-btn').forEach(btn => {
          btn.classList.toggle('active', btn.getAttribute('data-category') === item.category);
        });
      }
      playMovie(item);
    });
...
function playMovie(item) {
  ...
  if (playerWrapper && item.embedUrl) {
    // Płynne ładowanie iframe bez migania dzięki wbudowanemu zarządzaniu DOM
  ...
  // Automatyczny scroll do odtwarzacza
...
function closePlayer() {
  ...
  if (playerWrapper) {
    playerWrapper.innerHTML = `
      <img src="https://placehold.co/800x450/111111/FFFFFF?text=Wybierz+odcinek+z+listy+ponizej" alt="Podgląd" id="placeholder-img" loading="lazy">
    `;
  }
...
document.addEventListener('DOMContentLoaded', () => {
  // Wyłączenie loadera po załadowaniu
  setTimeout(() => {
  ...
  if (heroPlayBtn) {
    heroPlayBtn.addEventListener('click', () => {
      // Domyślnie odtwarza pierwszy element z aktywnej kategorii
      const list = contentData[currentCategory];
